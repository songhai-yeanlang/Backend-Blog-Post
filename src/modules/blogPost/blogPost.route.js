...
const { authMiddleware, validateQuery, isLogin } = require('../auth/auth.middleware');
const blogPostController = require('./blogPost.controller');
const { createBlogSchema, updateBlogSchema, getBlogsQuerySchema } = require('./blogPost.validation');
...
router.get('/get-all-blogs', validateQuery(getBlogsQuerySchema), blogPostController.getAllBlogs);
router.get('/get-owner-blogs', isLogin, validateQuery(getBlogsQuerySchema), blogPostController.getAllOwnerBlogs);
